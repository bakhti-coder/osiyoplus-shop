...
const SingleProducts: FC<{ data: ProductsType; pr: ProductsType[] }> = ({
  ...
  const sendData = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    formData.append("pro_name", sendProduct?.title);
    formData.append("pro_description", sendProduct?.description);
    formData.append("pro_price", sendProduct?.price);
    formData.append("pro_id", sendProduct?.id);
    ...
    <>
      ...
      <div className="flex justify-start flex-wrap md:flex-nowrap ">
        <div className="max-w-[600px]">
          <img
            src="https://images.uzum.uz/chk8bi36edfostif8620/original.jpg"
            alt="image"
        ...
        <div className="md:pt-0 pt-5 ml-0 md:ml-20 w-full">
          ...
          <p className="mt-3 font-medium text-[24px]">
            {" "}
            <span className="font-bold text-[26px]">Narxi:</span> {data.price} so'm
          </p>
            ...
            <Link key={idx} href={`/products/${product.id}`}>
              <div className="  border border-lightGray p-6 rounded-lg hover:scale-105 transition-transform ease-out duration-200">
                ...
                <div className="font-semibold flex items-center justify-between mt-4 mb-1">
                  <p className="w-32 truncate">{product.title}</p>
